    ...
    <div class="col-sm-9">
      <div class="card" style="width: 100%;">
        <div class="card-body" style="padding: 0 0;">
          ...
          <div class="card-body">
            <form action="{{ route('user.messSenderHandle') }}" method="POST" accept-charset="utf-8" accept-charset="utf-8" enctype="multipart/form-data">
             @csrf
            ...
            <div class="row">
              <div class="m-auto">
                ...
                @if (session('status-sendmess'))
                <div class="row alert alert-success">
                  <div class="col-md-12 text-center">
                    <h6 class="text-uppercase">{{ session('status-sendmess') }}</h6>
                  </div>
                </div>
                @elseif (session('failed-sendmess'))
                <div class="row alert alert-danger text-center">
                  <h6 class="text-uppercase">{{ session('failed-sendmess') }}</h6>
                </div>
                @endif
              </div>
            </div>

            @foreach ($data['dataReceiver'] as $value)
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-12">
                  <input type="text" class="form-control-plaintext text-ip-restyle" id="nguoi_nhan" value="{{ $value->tentaikhoan }}" name="nguoi_nhan" readonly>
              </div>
            </div>
            ...
            <div class="form-group row">
              @foreach ($data['dataProduct'] as $value)
                ...
                <div class="col-sm-12">
                  <input type="text" class="form-control-plaintext text-ip-restyle" id="tieu_de" name="tieu_de" value="Về :{{ $value->ten_sp }}">
                </div>
               @endforeach
            </div>
            ...
            <div class="form-group row float-right">
              <a title="Gửi tin" style="margin-right: 15px;"><button type="submit" class="btn btn-warning" >Gửi tin nhắn</button></a>
            </div>
